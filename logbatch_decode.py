    ...
    try:
        ...
        if str is not None and terse:
            ...
            for s in str:
                ...
                #    out = out+s
                #elif re.search('bugleMessageSource', s):
                #    out = out+s
                elif re.search('sendAttempt', s):
                    out = out+s
                #elif re.search('phoneNumberFormat', s):
                #    out = out+s
...
def decode_gboard_pb(bb, verbose=False, terse=False):
    f = open('/tmp/gboard_bytes', 'wb')
    f.write(bb)
    f.close()
    try:
        str = subprocess.check_output("cat /tmp/gboard_bytes | protoc --decode=LatinIME_LogEntry -I='"+mypath+"' gboard.proto3",shell=True,stderr=subprocess.STDOUT, text=True)
        if str is not None and terse:
            # just write out minimal event info
            str = str.split('\n')
            out = ""; ts = False
...
try:
    ...
    for i in range(len(logbatch.inner)):
        ...
        if inner.logSourceName == "LATIN_IME":
           try_decode_pb_array(None, inner.logEntry, decode_gboard_pb, verbose=False)
        else:
           continue
        ...
        #    try_decode_pb_array("logEntry", inner.logEntry, decode_dialer_pb, verbose=True)
           ...
           try_decode_pb_array("logEntry", inner.logEntry, decode_gboard_pb, verbose=True)
        else:
           try_decode_pb_array("logEntry", inner.logEntry, decode_pb)
        print("}")
